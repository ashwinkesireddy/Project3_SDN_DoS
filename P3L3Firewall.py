		...
		if(match.dl_type == packet.IP_TYPE):
		  ...
		  if ip_packet.protocol == ip_packet.TCP_PROTOCOL:
			log.debug("TCP it is !")
                        ...
                        print tcppkt.SYN,tcppkt.flags
                        if tcppkt.SYN:
                            if not match.dl_src in self.ptbl:
                                self.ptbl[match.dl_src]=[match.nw_src]
                            else:
                                self.ptbl[match.dl_src].append(match.nw_src)
                            print self.ptbl
                            if len(self.ptbl[match.dl_src])>5:
                                #print "not issuing any rule"
                                self.installFlow(event, 1, 
